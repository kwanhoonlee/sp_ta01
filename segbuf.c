				   ...
				   struct nilfs_write_info *wi, int mode)
{
	...
	// ADD BY LMS 2019.11.04
	if(bio->bi_bdev!=NULL && segbuf->sb_super!=NULL){	// submit_bio가 호출되기 직전에 코드를 작성했습니다. 커널이 NULL을 참조하지 않도록 예외처리를 진행합니다.
		bio->bi_bdev->bd_super = segbuf->sb_super;		// FS name은 super block이 갖고 있습니다. 따라서 nilfs2의 segbuf구조체의 super block을 
	}													// 실제 submit_bio가 진행되는 bio구조체의 nilfs2의 super block에 넣어줌으로써 nilfs2 FS name을 출력할 수 있습니다.			
	// END
	submit_bio(mode, bio);
	segbuf->sb_nbio++;
